...
const Sidebar = () => {
    ...
    return (
        <nav className="hidden lg:flex flex-col w-64
            ...
            <div className="flex flex-col gap-2">
                {navItems.map((item) => (
                    <NavLink key={item.path} to={item.path}
                        className={({ isActive }) => `flex items-cemter gap-3 px-4 py-2.5 border-l-3 transition-all duration-200 ${isActive ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400 font-medium' :
